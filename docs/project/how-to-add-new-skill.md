...
│   └── commands/             # 斜杠命令 (可选)
│       └── wgc.md           # /wgc 命令
├── skills/                   # 技能目录(历史结构,仅供参考)
│   ├── claude-notify/       # 示例技能(v3.0 起根级别扁平结构)
│   │   └── SKILL.md         # 技能实现
...
### Q: 可以使用子代理吗?
A: 可以。参考 `claude-notify` 技能中调用脚本与子任务的组织方式(如 `scripts/`、`claude-notify-setup/` 子技能)。
