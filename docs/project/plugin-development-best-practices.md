# Claude Code 技能开发最佳实践

## 概述

本文档总结了在 Claude Code 中开发技能的最佳实践,特别是避免命令重复和缓存问题的策略。
...
**策略**:
1. 在 `<skill-name>/` 目录中开发
2. **不要**添加到 marketplace.json
3. 使用本地路径直接加载
...
技能从多个位置被加载:
1. 本地开发目录: `<skill-name>/`
2. Claude 缓存: `~/.claude/plugins/cache/<project>/<skill-name>/`
3. Marketplace 工作区: `~/.claude/plugins/marketplaces/<project>/plugins/<skill-name>/`
...
```bash
# 删除 Claude Code 缓存
rm -rf "C:\Users\<username>\.claude\plugins\cache\<project>\<skill-name>"

# 删除 Marketplace 工作区
rm -rf "C:\Users\<username>\.claude\plugins\marketplaces\<project>\plugins\<skill-name>"
rm -f "C:\Users\<username>\.claude\plugins\marketplaces\<project>\.claude\commands\*.md"
```

**步骤 2: 调整 marketplace.json**
- **开发模式**: 从 marketplace.json 中移除技能配置
- **发布模式**: 从本地目录移除技能,只保留 marketplace.json 配置
...
**根本原因**:
技能内同时存在 `commands/` 和 `skills/` 定义

**解决方案**:
```bash
# 删除 commands/ 目录,只保留 skills/ 定义
rm -rf <skill-name>/commands/
```
...
# 检查缓存(应该不存在)
ls "C:\Users\<username>\.claude\plugins\cache\<project>\<skill-name>"

# 检查 marketplace(应该不存在或与本地一致)
ls "C:\Users\<username>\.claude\plugins\marketplaces\<project>\plugins\<skill-name>"
```
...
1. **创建技能目录**
```bash
mkdir -p <skill-name>/.claude-plugin
...
1. **本地开发目录**
```bash
find . -name "*.md" -path "*/skills/*" -not -path "./.gsd/*" -not -path "./node_modules/*"
```
...
确保每个技能只在以下位置之一存在:
- ✅ `<skill-name>/skills/<skill-name>/SKILL.md` (推荐)
- ❌ 不要同时在多个位置存在
...
3. **定期清理**: 结构变化时清理缓存
4. **扁平目录结构**: 每个技能在根目录拥有独立目录

### 检查清单

**开发新技能时**:
- [ ] 在根目录创建 `<skill-name>/` 目录
