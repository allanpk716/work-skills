...
用户期望只看到 /windows-git-commit 命令。

fix:
1. 删除旧位置的命令文件 `.claude/commands/wgc.md`
2. 将 `plugins/windows-git-commit/commands/wgc.md` 重命名为 `windows-git-commit.md`
现在命令将显示为 `/windows-git-commit`,符合用户期望

verification: 用户验证后确认只看到一个命令
files_changed: [".claude/commands/wgc.md", "plugins/windows-git-commit/commands/wgc.md → windows-git-commit.md"]
resolved_by: 重命名命令文件以匹配预期的命令名称
