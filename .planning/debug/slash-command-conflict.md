...
**第三层 (最终):** 插件内部重复定义
- `plugins/windows-git-commit/commands/windows-git-commit.md` (命令别名 → 显示为 /windows-git-commit:wgc)
- `plugins/windows-git-commit/skills/windows-git-commit/SKILL.md` (技能定义 → 显示为 /windows-git-commit)

fix:
1. 删除旧的 skills/ 目录
2. 删除根级别的 .claude/commands/ 和 .claude/hooks/
3. 删除插件内的 commands/ 目录
4. 只保留 plugins/windows-git-commit/skills/ 定义

最终状态:
- 只有 `plugins/windows-git-commit/skills/windows-git-commit/SKILL.md` 存在
- 命令显示为 `/windows-git-commit`
- 无重复定义

verification: 待用户重启 Claude Code 最终验证
files_changed: ["skills/", ".claude/commands/", ".claude/hooks/", "plugins/windows-git-commit/commands/"]
resolved_by: 完全清理重复定义,统一使用 skills/ 结构
