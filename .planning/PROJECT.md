...
**Goal:** 将多技能集合瘦身回归单一技能项目,只保留 claude-notify,移除 windows-git-commit 与 codepoint,并将 NPX 安装器裁剪为仅服务于 claude-notify。

**Target features:**
- 移除 windows-git-commit 技能目录及其全部内容
- 移除 codepoint 技能目录及其全部内容(含 8 个子技能、templates、references)
- 移除 docs/codepoint 文档与调研工作区(docs/claude-notify 保留)
- 裁剪 installer:剥离 git-ssh/git-user/TortoiseGit 检测与配置、多技能 marketplace 集成、完整卸载模块;保留 Windows 检测、Python/pip 检测、Pushover 配置、通知 hooks 注册、验证
- 更新根 README/CHANGELOG/package.json 为单一技能项目,版本升至 v3.0.0

**Key context:** claude-notify 自身代码本里程碑不动(仅作为回归测试对象);这是破坏性变更(2/3 技能下线),主版本号 v2.x → v3.0;不引入新功能,跳过 research 阶段。

## Requirements

### Active

<!-- v3.0 当前里程碑范围 — 瘦身回归单一技能项目。 -->

- [ ] 移除 windows-git-commit 与 codepoint 技能目录及其文档
- [ ] 裁剪 installer 仅服务 claude-notify(剥离 git/marketplace/uninstall 耦合代码)
- [ ] 更新根项目元数据(README/CHANGELOG/package.json)为单一技能项目
- [ ] 版本升至 v3.0.0 并同步 git tag,claude-notify 回归测试通过
...
---
*Last updated: 2026-06-26 — Milestone v3.0 (聚焦 claude-notify 重构) started*
