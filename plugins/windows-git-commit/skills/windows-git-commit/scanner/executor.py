...
def run_pre_commit_scan(repo_root: Path = None, use_colors: bool = None, lang: str = 'zh') -> Tuple[bool, List[ScanIssue]]:
    """
    ...
    5. Scan each file with Phase 6/7/8 rules
    6. Print report and return scan result

    Error Handling (Phase 8):
    - Scanner errors do NOT block commits (non-blocking)
    - Shows warning and returns success=True
    ...
    Args:
        ...
        lang: Language code for messages ('zh' or 'en'), defaults to 'zh'

    Returns:
        Tuple of (success: bool, issues: List[ScanIssue])
        - success: True if no issues found (or scanner error), False otherwise
            ...
            for rule in internal_rules:
                ...
                for match in matches:
                    ...
                    issues.append(create_issue(
                        ...
                        line_number=line_num,
                        content=matched_text,
                        suggestion='Internal info leaked - add whitelist: # gitcheck:ignore-line'
                    ))

        # 6. Print report and return results
        print_scan_report(issues, use_colors=use_colors, lang=lang)
