...
**v1.1 - Windows Git Commit 安全扫描:**
- [ ] 添加代码安全扫描功能,在提交前检测敏感信息
- [ ] 检测 AWS、Git 服务、通用 API 密钥等凭证
- [ ] 检测缓存文件、编译产物、临时文件
- [ ] 检测配置文件泄露(.env、credentials 等)
- [ ] 检测内部信息(内网 IP、内部域名、邮箱)
- [ ] 发现问题后阻止提交并显示详细提示
- [ ] 支持基于 .gitignore 的自定义规则
- [ ] 在 git commit 之前扫描暂存区内容
...
|----------|-----------|---------|
| 在 git commit 前扫描 | 能捕获已暂存的问题,避免进入版本历史 | — Pending |
| 复用 .gitignore 格式 | 无需学习新语法,用户熟悉 | — Pending |
| 阻止提交而非警告 | 强制用户处理安全问题,更安全 | — Pending |
| 显示详细问题信息 | 帮助用户快速定位和修复问题 | — Pending |
| 内置规则 + 自定义 | 覆盖常见场景,同时支持项目定制 | — Pending |
