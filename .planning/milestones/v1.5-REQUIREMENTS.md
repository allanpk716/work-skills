# Requirements Archive: v1.5 NPX Uninstall Support
...
- [x] **CLI-01**: User can run `npx @allanpk716/work-skills-setup --uninstall` to trigger the uninstall flow
- [x] **CLI-02**: `--help` output includes uninstall usage and description
- [x] **CLI-03**: `--version` output remains consistent with installer version

### 插件清理

- [x] **PLUG-01**: System detects which plugins are currently installed (claude-notify, windows-git-commit)
...
- [x] **ENV-01**: System detects Pushover environment variables set via setx (PUSHOVER_USER_KEY, PUSHOVER_API_TOKEN)
...
- [x] **UX-04**: Uninstall flow supports bilingual output (Chinese/English, auto-detected)
...
| PLUG-01 | Phase 24 | Complete |
| ENV-01 | Phase 24 | Complete |
